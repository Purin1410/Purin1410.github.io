...
| Case studies      | `src/content/cases/en/*.md`, `vi/*.md`            | Every project needs both locales. Most case studies use Markdown bodies; HMER is the exception and keeps only `project`, `locale` and `outcome` metadata because its visible narrative lives in `src/components/HmerResearchStory.astro`. |
...
HMER content-source exception: edit the visible English/Vietnamese research narrative in `src/components/HmerResearchStory.astro`. The HMER case Markdown files retain only the registry metadata and rendered `outcome`; editing a prose body there will not change the page.
